...
void solve(){
    ...
    //using sets in expensive hence use vectors for calculations for future purposes
    for(int i=0; i<n; i++){
    	...
    	if(start == end){
    		...
    		int dist = upper_bound(vis.begin(),vis.end(),end) - lower_bound(vis.begin(),vis.end(),start);
    		if(end - start + 1 > dist){
    			ans+='1';
    		}
    		else ans+='0';
